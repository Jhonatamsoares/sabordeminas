...
public interface OrderService {
    ...
    int addOrderToCart(String customerId, String adminId, List<Dish> content);
    ...
    // Admin check all active orders, no matter there is or isn't a driver
    List<Order> adminGetActiveOrders(String adminId);

    // Admin check order history
    List<Order> adminFindPastOrders(String adminId);

    int addComment(String id, int rating, String content);

    int deleteComment(String id);

    List<Comment> adminGetComments(String adminId);
}